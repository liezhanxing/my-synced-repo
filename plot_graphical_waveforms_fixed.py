...
try:
    ...
    for i in range(min(len(original_data['I']), len(decoded_data['I']))):
        ...
        i_match = "✓" if orig_i == decod_i else "✗"
        q_match = "✓" if orig_q == decod_q else "✗"
        print(f"{i}\t{orig_i}\t{decod_i}\t{orig_q}\t{decod_q}\t{i_match}\t{q_match}")
    ...
    print(f"\n总体匹配情况: {'完全匹配 ✓' if all_match else '存在差异 ✗'}")